...
- Warehouse address compatibility (2026-09-22): canonical UI sends `district = ''`, mapped to nullable storage by the existing service. Create/update supplied geo must be a complete finite numeric pair within latitude/longitude ranges and six-decimal precision. Address-changing PATCH requires a coordinate pair; metadata-only updates retain legacy address/geo. Coordinate omission on create remains API-compatible; the Admin UI requires explicit map confirmation. No historical backfill or lifecycle/ranking change.
