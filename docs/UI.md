...
- **Viewport center != selected coordinate**: focus không tạo marker, không ghi lat/lng, không bật Confirm khi chưa chọn điểm. Initial coordinate phải finite/đủ cặp/trong range; invalid thì fallback. Chỉ áp dụng viewport khi mount mỗi lần mở picker; pan/zoom/click/drag hoặc rerender với cùng fingerprint không tự recenter. Mở lại ưu tiên tọa độ đã xác nhận; Hủy giữ nguyên giá trị cũ.
- `address-location-model.ts` là nguồn duy nhất cho fingerprint: NFC, trim, lowercase, gộp whitespace, theo thứ tự street/ward/district/city; không đưa contact/phone/label vào fingerprint. Confirm lưu coordinate + fingerprint trong form state. Đổi địa chỉ làm coordinate stale, ẩn marker cũ, đóng phiên map/discard draft nếu đang mở và hiện “Địa chỉ đã thay đổi. Vui lòng chọn lại vị trí trên bản đồ.”. Saved Address có coordinate stale bị chặn lưu đến khi reconfirm (không PATCH omission vì sẽ giữ coordinate DB cũ). Delivery vẫn optional: serializer Shipment/Quote bỏ cặp coordinate stale/missing proof; chỉ gửi cặp đã confirm cho địa chỉ hiện tại. Fingerprint không gửi ra API.
...
- Giữ `district` trong DB/API; địa chỉ mới gửi `''`, không dựng district giả. Edit hiển thị district cũ readonly; đổi province/ward bỏ district cũ. Contact-only edit giữ nguyên các chuỗi legacy và tọa độ; nếu thay địa chỉ không khớp dataset thì yêu cầu chọn hierarchy hiện tại. Không tự chuyển legacy ward sang ward mới theo tên gần giống, không sửa Shipment snapshot lịch sử.
- Browser regression: `node frontend/test/address-location/check.mjs` build production fixture dùng các page thật và API mock, kiểm tra request payload nhưng không chứng minh real DB/staging persistence. Các regression map: `node frontend/test/location-picker/viewport.mjs`, `node frontend/test/location-picker/tiles.mjs --production --offline`.
