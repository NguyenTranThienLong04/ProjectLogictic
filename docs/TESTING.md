...
- Consistency regression (2026-09-25): after `npm run build`, run `node frontend/test/address-location/check.mjs --validated-search`. This loads the real compiled backend search service with raw provider fixtures before rendering results in the production-built frontend. Covers `123 Nguyễn Trãi + Hồ Chí Minh + Bến Thành`, wrong province/ward, Thủ Đức even with a conflicting matching ward, valid exact coordinates/map focus, unchanged selectors, empty/error/manual fallback and stale requests across the existing four viewports. Provider/HTTP persistence/tiles remain mocked; it is not live geocoding or database evidence. Service tests also cover aliases, missing proof, bare/explicit conflicting hierarchy, canonical query/viewbox and accepting Thủ Đức when actually selected.
