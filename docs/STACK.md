...
- Consistency fix (2026-09-25): search now requires a canonical province + ward pair, resolved at the backend from `backend/src/common/addresses/administrative-data.ts`. Frontend selectors re-export that same pure catalogue; the vendored JSON/license/provenance moved alongside it (no duplicate dataset). The pinned ward `fullName` is retained for correct Phường/Xã/Đặc khu labels. Query is street + canonical full ward name + canonical province + Vietnam; legacy district cannot change it. `viewbox` reuses the ward centre with a ±0.05° latitude / cosine-adjusted longitude window, `bounded=0` (bias only: the dataset has no polygons/bounds). Backend requires affirmative VN/province/ward evidence and rejects contradictory structured/display hierarchy before returning coordinates. Unknown/missing administrative proof fails closed to no results/manual pin. See `docs/LOCATIONIQ_ADDRESS_CONSISTENCY.md` for request examples and regressions.
